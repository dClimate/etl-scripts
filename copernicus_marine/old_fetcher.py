...
class CopernicusOcean(Fetcher, Logging):
    ...
    def trigger_reanalysis_download(self, current_datetime: datetime.datetime) -> datetime.datetime:
        ...
        if current_dataset:
            previous_reanalysis_finalization_date = (
                numpydate_to_py(
                    datetime.datetime.strptime(current_dataset.attrs["reanalysis_end_date"], "%Y%m%d")
                )
            ...
            previous_interim_end_date = (
                numpydate_to_py(
                    datetime.datetime.strptime(current_dataset.attrs["interim_reanalysis_end_date"], "%Y%m%d")
                )
    ...
    def set_zarr_metadata(self, dataset: xr.Dataset):
        ...
        if not hasattr(self, "reanalysis_end_date"):
            self.find_reanalysis_end_dates()
        dataset.attrs["reanalysis_end_date"] = datetime.datetime.strftime(self.reanalysis_end_date, "%Y%m%d")
        dataset.attrs["interim_reanalysis_end_date"] = datetime.datetime.strftime(
            self.interim_reanalysis_end_date, "%Y%m%d"
        )
