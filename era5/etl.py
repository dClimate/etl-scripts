...
hamt: HAMT
if num_ds == 0:
    hamt = HAMT(store=IPFSStore(rpc_uri_stem=f"http://127.0.0.1:{rpc_port}"), max_cache_size_bytes=CACHE_SIZE)
    print(f"Writing part {num_ds} to IPFS")
    print(ds)
    ds.to_zarr(store=hamt)
else:
    with Path(f"/srv/ipfs/etl-scripts/era5/{data_var}-part{num_ds-1}.cid").open("r") as f:
        original_ds_cid_str = f.read().rstrip()
    original_ds_cid = CID.decode(original_ds_cid_str)
    hamt = HAMT(store=IPFSStore(rpc_uri_stem=f"http://127.0.0.1:{rpc_port}"), max_cache_size_bytes=CACHE_SIZE, root_node_id=original_ds_cid)
    print(f"Writing part {num_ds} to IPFS")
    print(ds)
